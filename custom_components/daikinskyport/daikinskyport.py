...
class DaikinSkyport(object):
    ...
    def get_sensors(self, index):
        ...
        sensors.append({"name": f"{name} Outdoor heating", "value": round(thermostat['ctOutdoorHeatRequestedDemand'] / DAIKIN_PERCENT_MULTIPLIER, 1), "type": "demand"})
        ...
        sensors.append({"name": f"{name} Indoor liquid", "value": ((thermostat['ctAHLiquidTemperature']/10)-32)*(5/9), "type": "temperature"})
        sensors.append({"name": f"{name} Outdoor liquid", "value": ((thermostat['ctOutdoorLiquidTemperature']/10)-32)*(5/9), "type": "temperature"})
